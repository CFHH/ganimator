...
def degeuler2mat(rotations):
    #输入shape = (帧数，骨骼数，3)，单位是角度
    #输出shape = (帧数，骨骼数，9)
    frames = rotations.shape[0]
    rotations = np.radians(rotations)
    rotations = R.from_rotvec(rotations.reshape(-1, 3)).as_matrix().reshape(frames, -1, 9)
    return rotations
...
def mat2degeuler(rotmats):
    #输入shape = (帧数，骨骼数，9)
    #输出shape = (帧数，骨骼数，3)，单位要转成角度
    frames = rotmats.shape[0]
    rotmats = rotmats.reshape(frames, -1, 3, 3)
    rotmats = get_closest_rotmat(rotmats)
    rotations = R.from_matrix(rotmats.reshape(-1, 3, 3)).as_rotvec().reshape(frames, -1, 3)
